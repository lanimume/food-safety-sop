...
class AuthGuard {
    ...
    renderNav() {
        ...
        items.forEach(item => {
            ...
            a.href = item.href;
            a.target = '_self';
            a.className = `nav-item ${isActive ? 'active bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'} w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all`;
    ...
    renderUserInfo() {
        ...
        const avatarEl = document.querySelector('.w-9.h-9.bg-indigo-100');
        ...
        if (avatarEl) {
            ...
            avatarEl.className = `w-9 h-9 ${this.user.role === 'admin' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-blue-700'} rounded-full flex items-center justify-center font-bold text-sm`;
        }
    }
