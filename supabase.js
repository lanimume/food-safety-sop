...
// ==================== Auth 用户管理（管理员用）====================
async function createAuthUser(email, password, username, personId) {
    if (!isSupabaseReady()) {
        showToast('离线模式', '无法创建用户', 'warning');
        return null;
    }
    try {
        ...
        handleSupabaseError('createAuthUser', e);
        return null;
    }
}
...
async function deleteAuthUser(userId) {
    ...
    try {
        ...
        handleSupabaseError('deleteAuthUser', e);
        return false;
    }
}
